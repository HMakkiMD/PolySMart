...
# === PBAT: 1CCH4 ===
rep_pbat = process_cluster(bead_type_index=4, bead_type_name='1CCH4')
...
for m in range(num_cluster):
    for k in range(Length_LC):
        ...
        if bead_id != 0:
            matches = np.where(all_bead_no == bead_id)[0]
            for i in matches:
                if str(atoms[i, 3]) == 'CLD':
                    rep_cre[k, m] = int(atoms[i, 0])

count_cre = np.count_nonzero(rep_cre, axis=0) / 18  # replace 18 (total number of crosslinkers beads) if needed
...
# === Separate PE and P3HB interface beads ===
m_pe = []
m_p3hb = []
