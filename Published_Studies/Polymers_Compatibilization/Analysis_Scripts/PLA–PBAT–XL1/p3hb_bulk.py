...
def count_row_occurrences_to_excel(file_name, output_file):
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl', mode='a') as writer:
        df.to_excel(writer, sheet_name='p3hb_bulk', index=False, header=False)

# Specify the file names
input_file = 'p3hb_bulk.txt'
...
print(f"Data has been written to the 'p3hb_bulk' sheet in '{output_file}'.")
