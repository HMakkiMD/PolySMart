...
for i in range(bond_line+1,ang_line):
    if lines[i].split()[4] in ['25000','12000']:
        cl_bonds.append([lines[i].split()[0],lines[i].split()[1]])

pbat_cls = []   # list of reacted P3HB beads and their molecule number
...
for i in range(3,bond_line):
    if lines[i].split()[4] == '1CCH4':
        pbat_cls.append([lines[i].split()[0],lines[i].split()[2]])
...
pla_udc_pbat , pla_udc_pla , pbat_udc_pbat , loops, counter = 0,0,0,0,0
pbat_mols , pla_mols = [],[]
# find double reacted UDCs and to which polymer they are connected
for i in pbat_cls:
    pbat_mols.append(i[1])
for i in pla_cls:
    pla_mols.append(i[1])
    ...
    for j in range(i+1,len(mol_bonds)):
        if mol_bonds[i][0] == mol_bonds[j][0]:
            ...
            if (mol_bonds[i][1] in pla_mols) and (mol_bonds[j][1] in pla_mols):
                pla_udc_pla += 1
            elif (mol_bonds[i][1] in pbat_mols) and (mol_bonds[j][1] in pbat_mols):
                pbat_udc_pbat += 1
            elif ((mol_bonds[i][1] in pla_mols) and (mol_bonds[j][1] in pbat_mols)) or \
                 ((mol_bonds[i][1] in pbat_mols) and (mol_bonds[j][1] in pla_mols)):
                pla_udc_pbat += 1
            #break
'''
a = []
...
with open(f'{file_name}_results.txt','w') as f:
    f.write('|=================================================|\n')
    f.write('|  No. of PBAT-CL-PBAT connections   =   %7d  |\n' %(pbat_udc_pbat))
    f.write('|  No. of  PLA-CL-PLA  connections   =   %7d  |\n' %(pla_udc_pla))
    f.write('|  No. of PBAT-CL-PLA  connections   =   %7d  |\n' %(pla_udc_pbat))
