...
def edit_gro_itp(XL, gro_filename, itp_filename, loop_x):
    ...
    if ex != []:
        ...
        for m in range(len(xlfilelist2)-1,-1,-1):
            ...
            for n in range(len(bonds_list)):
                if (xlfilelist2[m].split()[1] == bonds_list[n].split()[0] and \
                    ...
                    temp1 = bonds_list[n].split()[1]
                    del_bond.append([xlfilelist2[m].split()[1],temp1])
                elif (xlfilelist2[m].split()[1] == bonds_list[n].split()[1] and \
                ...
                elif (xlfilelist2[m].split()[3] == bonds_list[n].split()[1] and \
        ...
        for n in range(len(delete)):
            xlfilelist2.pop(delete[n]) # delete repetitious beads from reacting pairs list
            del_bond.pop(delete[n])

        for m in range(len(xlfilelist2)):
            exch_items.append((xlfilelist2[m].split()[1],xlfilelist2[m].split()[3])) # add reacting pairs to a list
            exch_itemslist.append(xlfilelist2[m].split()[1]) # add reacting beads to a list
            exch_itemslist.append(xlfilelist2[m].split()[3]) # add reacting beads to a list

        xlfilelist += xlfilelist2
            ...
            while ('[' not in filelist[j+1]) and (j+1 != end_line):
                for k in range(len(del_bond)):
                    if (filelist[j+1].split()[0] == del_bond[k][0] and \
                       ...
                       (filelist[j+1].split()[0] == del_bond[k][1] and \
    ...
    total_items = items + exch_items
    total_itemslist = itemslist + exch_itemslist
    ...
    for q in range(len(bonds_list)):
        ...
        for l in range(len(total_items)):
            for x in range(2):
                if total_items[l][x] == sp[0]:
                    bonds_found.append((sp[1],sp[0]))
                    marked_beads.append(sp[1])
                elif total_items[l][x] == sp[1]:
                    bonds_found.append((sp[0],sp[1]))
                    marked_beads.append(sp[0])
#    marked_beads = list(set(marked_beads)-set(total_itemslist).intersection(set(marked_beads)))
    for q in range(len(bonds_list)):
        sp = bonds_list[q].split()
        for l in range(len(marked_beads)):
            if ((marked_beads[l] == sp[0]) and (sp[1] not in total_itemslist)) or \
                ((marked_beads[l] == sp[1]) and (sp[0] not in total_itemslist)):
            ...
            if (bond_line != 0 and ex_bonds[x] != []):
                for y in range(bond_line+1,bond_end):
                    ...
                    if sorted(filelist[y].split()[:2]) in list(sorted(bonds_found[z]) for \
                        ...
                        if (ex_bonds[x][0] == filelist[int(filelist[y].split()[0])+2].split()[4] and \
                            ex_bonds[x][1] == filelist[int(filelist[y].split()[1])+2].split()[4]) or \
                           (ex_bonds[x][1] == filelist[int(filelist[y].split()[0])+2].split()[4] and \
                            ex_bonds[x][0] == filelist[int(filelist[y].split()[1])+2].split()[4]):
    ...
    for x in range(len(items)):
        for y in range(4, len(lis)): # find the bead types of the pair beads after reaction
            if (lis[y][0] == items[x][2]) and (lis[y][1] == items[x][3]) or \
                ...
                break
    if ex != []:
        for x in range(len(exch_items)):
            filelist[int(exch_items[x][0])+2].split()[4]
            for y in range(len(ex)): # find the bead types of the pair beads after reaction
                if (ex[y][0] == filelist[int(exch_items[x][0])+2].split()[4]) and (ex[y][1] == filelist[int(exch_items[x][1])+2].split()[4]) or \
                   (ex[y][0] == filelist[int(exch_items[x][1])+2].split()[4]) and (ex[y][1] == filelist[int(exch_items[x][0])+2].split()[4]):
                    bond_dist = ex[y][5]
                    bond_const = ex[y][6]
                    filelist.insert(insert_point+bond_nos, '%7s  %7s  %7s  %7s  %7s\n' \
                            %(exch_items[x][0], exch_items[x][1], '1', bond_dist, bond_const))
                    bond_nos += 1
                    break
            ...
            if (angle_line != 0 and ex_angs[x] != []):
                for y in range(angle_line+bond_nos+1,insert_point):
                    ...
                    if list(value for value in filelist[y].split()[:3] if value in \
                        ...
                        if (ex_angs[x][0] == filelist[int(filelist[y].split()[0])+2].split()[4] and \
                            ex_angs[x][1] == filelist[int(filelist[y].split()[1])+2].split()[4] and \
                            ex_angs[x][2] == filelist[int(filelist[y].split()[2])+2].split()[4]) or \
                           (ex_angs[x][2] == filelist[int(filelist[y].split()[0])+2].split()[4] and \
                            ex_angs[x][1] == filelist[int(filelist[y].split()[1])+2].split()[4] and \
                            ex_angs[x][0] == filelist[int(filelist[y].split()[2])+2].split()[4]):
    ...
    for l in range(len(bonds_found)):
        for x in range(len(total_items)):
            if bonds_found[l][1] == total_items[x][0]:
                temp = total_items[x][1]
            elif bonds_found[l][1] == total_items[x][1]:
                temp = total_items[x][0]
        one = filelist[int(bonds_found[l][0]) + 2].split()[4]
        two = filelist[int(bonds_found[l][1]) + 2].split()[4]
        three = filelist[int(temp) + 2].split()[4]
            ...
            if (dihedral_line != 0 and ex_dihs[x] != []):
                for y in range(dihedral_line+bond_nos+counter+1,insert_point):
                    ...
                    if list(value for value in filelist[y].split()[:4] if value in \
                        ...
                        if (ex_dihs[x][0] == filelist[int(filelist[y].split()[0])+2].split()[4] and \
                            ex_dihs[x][1] == filelist[int(filelist[y].split()[1])+2].split()[4] and \
                            ex_dihs[x][2] == filelist[int(filelist[y].split()[2])+2].split()[4] and \
                            ex_dihs[x][3] == filelist[int(filelist[y].split()[3])+2].split()[4]) or \
                           (ex_dihs[x][3] == filelist[int(filelist[y].split()[0])+2].split()[4] and \
                            ex_dihs[x][2] == filelist[int(filelist[y].split()[1])+2].split()[4] and \
                            ex_dihs[x][1] == filelist[int(filelist[y].split()[2])+2].split()[4] and \
                            ex_dihs[x][0] == filelist[int(filelist[y].split()[3])+2].split()[4]):
    ...
    try:
        ...
        for l in range(len(bonds_found2)):
            ...
            for x in range(len(bonds_found)):
                if bonds_found[x][0] == three:
                    ...
                    break
            for x in range(len(total_items)):
                if total_items[x][0] == two:
                    one = total_items[x][1]
                    break
                elif total_items[x][1] == two:
                    one = total_items[x][0]
                    break
        ...
        for l in range(len(bonds_found)):
            ...
            for x in range(len(total_items)):
                if total_items[x][0] == three:
                    two = total_items[x][1]
                    break
                elif total_items[x][1] == three:
                    two = total_items[x][0]
                    break
            for x in range(len(bonds_found)):
        ...
        for l in range(len(dih_list)):
            first  = filelist[int(dih_list[l][0]) + 2].split()[4]
            second = filelist[int(dih_list[l][1]) + 2].split()[4]
            third  = filelist[int(dih_list[l][2]) + 2].split()[4]
            fourth = filelist[int(dih_list[l][3]) + 2].split()[4]
